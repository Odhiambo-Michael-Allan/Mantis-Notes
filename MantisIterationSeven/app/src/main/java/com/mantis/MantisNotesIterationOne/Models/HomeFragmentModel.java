...
public class HomeFragmentModel extends FragmentModel {
    ...
    protected void deleteTableReference( NoteReference referenceToBeRemoved ) {
        noteRepository.deleteHomeNoteReference( referenceToBeRemoved.getNoteReferenceId() );
        notesViewModel.deleteFrequentFragmentNoteReference( referenceToBeRemoved
                .getNoteReferenceId() );
    }

    @Override
    public void syncWith( List<Note> notesList ) {
        Logger.log( "SYNCING WITH NOTES LIST.." );
        Iterator i = notesList.iterator();
        while ( i.hasNext() ) {
            Note note = ( Note ) i.next();
            boolean noteIsNotNew = noteInList( note.getId() ) ||
                    notesViewModel.noteInArchives( note.getId() ) ||
                    notesViewModel.noteInTrash( note.getId() );
            if ( !( noteIsNotNew ) ) { // This is a new Note..
                Logger.log( "INSERTING NEW NOTE" );
    ...
    public void editNote( int noteId, Note placeHolder ) {
        ...
        Note currentNote;
        try {
            currentNote = noteRepository.getNotesById( new int[] { noteId } ).
                    getValue().get( 0 );
